...
const StylesRoute = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (entry?.style_reference?.id !== null) {
      document
        .getElementById(`style-${entry?.style_reference?.id}`)
        ?.scrollIntoView({
          behavior: "auto",
          block: "center",
        });
  ...
  return (
    <>
      ...
      <div className="flex flex-col w-full h-full justify-start items-center px-6 overflow-auto">
              ...
              className={clsx(
                "flex relative aspect-[2/3] hover:scale-[1.04] transition-all hover:z-10 hover:ring-4 hover:ring-accent hover:rounded-lg border border-light-divider dark:border-dark-divider rounded-lg",
                {
                  "ring-2 ring-accent rounded-lg":
                    entry?.style_reference &&
                    entry?.style_reference.id == index.toString(),
                },
                {
